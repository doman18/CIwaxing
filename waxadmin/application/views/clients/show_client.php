...
if ($client) {
	foreach ($client as $key) {
		echo '<h3>'.$key['client_name'].' '.$key['client_surname'].' Nr:'.$key['client_number'].' <a href="'.base_url('clients/edit_client/'.$key['client_id']).'">Edytuj</a> <a href="'.base_url('clients/delete_client/'.$key['client_id']).'">Usuń</a></h3>';

		 echo 'Telefon: '.$key['client_phone'].'</br>';
		 echo 'Email: '.$key['client_mail'].'</br>';
		 echo 'Adres '.$key['client_address'].'</br></br>';
		 echo 'Hasło: '.$key['client_password'].'</br>';
